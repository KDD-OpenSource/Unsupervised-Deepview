...
#helper functions
def where_equal(a,b): # returns indices of elementwise equal np arrays
...
class DeepView:
	...
	def __init__(self, pred_fn, classes, max_samples, batch_size, data_shape, n=5,
				 lam=0.65, resolution=100, cmap='gist_rainbow', interactive=True, verbose=True,
		...
		inv_mapper : object
			An objchoirect that maps samples from the 2D space to the data input domain. The object
	...
	def _init_plots(self):
		...
		self.sample_plots = []
		plot = self.ax.plot([], [], 'o', label="uncertain",
				color=(0.0, 1.0, 1.0, 1.0), fillstyle='none', ms=12, mew=2.5, zorder=3, picker=mpl.rcParams['lines.markersize'])
		self.uncertain_plot = plot[0]
		#print(self.cmap(self.n_classes))

		for c in range(self.n_classes+1):
			if c == self.n_classes:
				continue
			color = self.cmap(c/(self.n_classes))
			#print(color)
	...
	def mle_single(self, data, x, k=20):
		...
		k = min(k, len(data) - 1)
		f = lambda v: - k / np.sum(np.log(v + 1e-7 / (v[-1] + 1e-7)))  # avoid divide by zero
		a = cdist(x, data)
	...
	def _predict_batches(self, x):
		...
		for i in range(0, n_inputs, self.batch_size):
			...
			batch = x[i:n_preds]
			#batch_reshaped = batch.reshape(batch.shape[0], 64)
		...
		#print(np.amax(hdboutliers.outlier_scores_))
		#print(np.amin(hdboutliers.outlier_scores_))
		threshold = pd.Series(hdboutliers.outlier_scores_).quantile(0.95)
		outliers = np.where(hdboutliers.outlier_scores_ > threshold)[0]
		hdb_labels = np.array([0] * len(hdboutliers.outlier_scores_))
		if np.mean(self.lids_of_x[outliers]) >= np.mean(self.lids_of_x[~outliers]): #higher LIDs adversarials
			...
			hdb_labels.fill(1)
		for i in outliers:
			hdb_labels[i]=self.which_label_outlier
		plt.scatter(umap_embed[:, 0], umap_embed[:, 1], c=hdb_labels, s=0.1, cmap='Spectral');
	...
	def queue_samples(self, samples, labels, preds, uncerts):
		'''
		...
		self.y_uncerts = np.concatenate((uncerts, self.y_uncerts))[:self.max_samples]
	...
	def add_samples(self, samples, labels):
		...
		pot_uncerts= not_equal(Y_preds, labels)
		#----------------------------------------------------------
		batch_reshaped = samples.reshape(samples.shape[0], 3072)  # potentially change with data shape
		self.queue_samples(samples, labels, Y_preds, self.y_uncerts)
		helper = np.full(len(self.y_uncerts), self.which_label_outlier)
		rec_uncerts = where_equal(self.y_uncerts, helper)
		# intersection real uncerts labelwise and recognized
		#print(np.setxor1d(pot_uncerts, det_uncerts))
		det_uncerts = np.intersect1d(pot_uncerts, rec_uncerts)
		# ---
		### precision score
		if self.which_label_outlier ==0:
			helper.fill(1)
		for element in pot_uncerts:
			helper[element] = self.which_label_outlier
		Y_true_roc = helper# potential uncerts real labels
		precision = precision_score(Y_true_roc, self.y_uncerts)
	...
	def compute_grid(self):
		'''
		Computes the visualisation of the decision boundaries. #TODO change
		'''
		if self.verbose:
		...
		print(mesh_max_class)

		# get color of gridpoints
		color = self.cmap(self.mesh_classes/mesh_max_class) #TODO correct?
	...
	def show(self):
		...
		#	decis_val =0
		uncertain = np.empty((0, 2), float)
		print(self.y_uncerts.shape)
		print("embeddingshape")
		print(self.embedded.shape)
		for c in range(self.n_classes):
			data = self.embedded[self.y_true==c]
			#print(self.y_true == c)
			###########mark datapoints which we deem uncertain
			data_uncert = self.y_uncerts[self.y_true == c] == self.which_label_outlier #outliers, uncertainties
			#print(data_uncert)
			uncertain_data = data[data_uncert]
			certain_data = data[~data_uncert]
			self.sample_plots[c].set_data(certain_data.transpose())
			uncertain = np.append(uncertain, uncertain_data, axis=0)
		...
		#print(uncertain)
		self.uncertain_plot.set_data(uncertain.transpose())
