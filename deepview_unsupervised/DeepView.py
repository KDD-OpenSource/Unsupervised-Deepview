...
class DeepView_unsupervised:
	...
	def _predict_batches(self, x):
		...
		for i in range(0, n_inputs, self.batch_size):
			...
			batch = x[i:n_preds]
			batch_reshaped = batch.reshape(batch.shape[0], batch.shape[1]* batch.shape[2])#* batch.shape[3]) # potentially change with data shape
	...
	def compute_grid(self): #TODO tochange
		...
		#----------------------------------------------
		X_reshaped = grid_samples.reshape(grid_samples.shape[0], grid_samples.shape[1]* grid_samples.shape[2]) # *grid_samples.shape[3])
	...
	def show_sample(self, event):
		...
		if hasattr(event, 'artist'):
			...
			sample, p, t = self.get_artist_sample(point)  #y_pred is model prediction ,y_uncerts outliers
			#title = 'uncertain' if p != t else 'certain' # if self.y_uncerts == self.which_label_outlier = 1
			title= 'certain' if t == self.which_label_outlier else 'uncertain' ## TOCHECK t really correct
			title= title
